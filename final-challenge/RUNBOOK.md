...
## STEP 4: Jenkins pipeline (build → push → deploy)
Use the Jenkinsfile pattern from before, pointed at ECR instead of Docker Hub:
- Checkout
- Build user-service image
- `aws ecr get-login-password | docker login` + push
- `kubectl set image deployment/user-service -n abhi-ejaz user-service=<new-image>`
- `kubectl rollout status deployment/user-service -n abhi-ejaz --timeout=90s`
